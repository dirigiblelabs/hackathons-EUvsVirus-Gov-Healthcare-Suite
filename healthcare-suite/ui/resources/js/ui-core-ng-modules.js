...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/healthcare-suite/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/healthcare-suite/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/healthcare-suite/api/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v4/js/healthcare-suite/api/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/healthcare-suite/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.healthcare-suite.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.healthcare-suite.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v4/web/healthcare-suite/ui/resources/templates/sidebar.html'
	}
}])
.directive('tiles', ['Tiles', function(Tiles){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/healthcare-suite/ui/resources/templates/tiles.html'
	}
}])
